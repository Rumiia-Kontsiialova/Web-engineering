...
SELECT 
	purchase_order_id,
	MIN(unit_cost) OVER(PARTITION BY purchase_order_id) AS min_init_cost,
    MAX(unit_cost) OVER(PARTITION BY purchase_order_id) AS max_init_cost,
    AVG(unit_cost) OVER(PARTITION BY purchase_order_id) AS avg_init_cost
FROM 
...
SELECT 
	purchase_order_id,
	SUM(quantity * unit_cost) AS multiply_quantity_unit_cost
FROM 
	purchase_order_details
GROUP BY 
	purchase_order_id;
